...
const ProductCard = ({
  ...
  if (product.is_bestseller) {
    ...
    badge = { text: subcategoryName, color: 'bg-purple-100 text-purple-800' };
  }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-100 hover:border-gray-200 group flex flex-col h-full">
      ...
      <div className="p-4 flex flex-col flex-grow">
        {/* Categoría principal resaltada + subcategoría diferenciada */}
        <div className="mb-3">
          {categoryName && (
            <div className="mb-1">
          ...
          {(product.is_bestseller || product.is_new) && badge ? (
            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs ${badge.color} border border-current`}>
              {badge.icon && <badge.icon size={12} className="mr-1" />}
              {badge.text}
            </span>
          ) : subcategoryName ? (
            ...
            <div className="flex items-center gap-2">
              {product.supplier.logo_url ? (
                <img
                  ...
                  onError={(e) => {
                    e.target.src = 'https://placehold.co/32x32/e5e7eb/6b7280?text=S';
                  }}
                />
              ) : (
