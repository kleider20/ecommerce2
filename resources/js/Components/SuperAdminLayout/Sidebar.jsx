...
import {
  Home, Building2, Users, Activity, DollarSign, BarChart3, Settings, Shield, X, Package,
  Plus, List, Store, User, Globe, FileText, Cog, HandCoins, MonitorCog, UserRoundCog
} from 'lucide-react';
import { Link, usePage } from '@inertiajs/react'; // ✅ Importa route

// ✅ Define los items SIN la ruta resuelta
const menuItemsConfig = [
  {
    id: 'dashboard',
    label: 'Dashboard',
    icon: Home,
    routeName: 'dashboard', // 👈 Nombre de la ruta, no la URL
    permission: null
  },
   {
    id: 'roles',
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  // ✅ Genera las rutas reales DURANTE el renderizado
  const menuItems = menuItemsConfig.map(item => {
    const routeUrl = route(item.routeName);
    const submenu = item.submenu?.map(sub => ({
      ...sub,
      route: route(sub.routeName)
    }));
    return {
      ...item,
      route: routeUrl,
  ...
  useEffect(() => {
    ...
    menuItems.forEach(item => {
      if (item.submenu && item.submenu.some(sub => route().current(sub.routeName))) {
        newOpen.add(item.id);
      }
    });
    if (newOpen.size !== openSubmenus.size || ![...newOpen].every(v => openSubmenus.has(v))) {
    ...
    <div className={`fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 shadow-2xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <nav className="mt-8 px-4 space-y-2">
        {visibleItems.map((item) => {
          const hasSubmenu = item.submenu;
        //   const isActive = hasSubmenu
        //     ? isSubmenuActive(item.submenu) || isRolesRoute()
        //     : route().current(item.routeName); // 👈 Usa routeName para verificar
        const isActive = item.id === 'roles'
            ? isRolesRoute()
            : hasSubmenu
                ? isSubmenuActive(item.submenu)
                  ...
                  <div className="ml-6 mt-1 space-y-1">
                    {item.submenu.map((subItem) => {
                      ...
                      return (
                        <Link
                          key={subItem.id}
                          href={subItem.route} // 👈 URL ya generada
                          className={`flex items-center px-4 py-2 text-xs rounded-lg ${
                            isSubActive
                              ? 'bg-emerald-600 text-white'
                              : 'text-gray-300 hover:bg-gray-800'
          ...
          return (
            <Link
              key={item.id}
              href={item.route} // 👈 URL ya generada
              className={`w-full flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 ${
                isActive
                  ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white shadow-lg'
                  : 'text-gray-300 hover:bg-gray-800 hover:text-white'
